...
class maestro extends Conectar
{
     ...
     private $trae_practicas;
     private $trae_practicas_es;
      private $trae_practicas_ge_es;
    ...
    public function __construct()
    {
        ...
        $this->trae_practicas=array();
        $this->trae_practicas_es=array();
        $this->trae_practicas_ge_es=array();
    }
   ...
   public function actualizar_maes()
    ...
    public function trae_practicas_ge($cv) 
        {
           $sql="select 
                       *
                         from 
             ...
             while($reg=mysql_fetch_assoc($res))
             {
                $this->trae_practicas[]=$reg;
             }
                return $this->trae_practicas;
        }
    public function actualizar_statusp()
    {
        parent::conexion();
          $sql=sprintf
            (
                ...
                cve_practica=%s
                ",
                parent::comillas_inteligentes($_POST["cv_p"])
            );
             mysql_query($sql);
             header("Location: ".Conectar::ruta()."c-mini/e-3/");exit;  
    } 
    public function trae_practicas_es($cvp) 
        {
           $sql="select 
                       *
                         from 
                               practicas 
                                        where cve_practica=$cvp
                               ;";
             $res=mysql_query($sql,parent::conexion());
             while($reg=mysql_fetch_assoc($res))
             {
                $this->trae_practicas_es[]=$reg;
             }
                return $this->trae_practicas_es;
        }
    public function npractica()
    {
           ...
           parent::conexion();
             $query=sprintf
                    (
                        ...
                        (null,%s,%s,0,%s,%s,%s,1);
                        ",
                        parent::comillas_inteligentes($_POST["cv_m"]),
                        parent::comillas_inteligentes($_POST["img"]),
                        parent::comillas_inteligentes($_POST["fcha"]),
                        parent::comillas_inteligentes($_POST["des"]),
                        parent::comillas_inteligentes($_POST["gpo"])
                    );
                    mysql_query($query);
                    header("Location: ".Conectar::ruta()."c-mini/e-3/");exit; 
        }
        public function trae_practicas_ge_es($cvp) 
        {
           $sql="select 
                       *
                         from 
                               practicas_realizadas,alumnos 
                                        where practicas_realizadas.cve_practica=$cvp and alumnos.cve_alum=practicas_realizadas.cve_alum 
                               ;";
             $res=mysql_query($sql,parent::conexion());
             while($reg=mysql_fetch_assoc($res))
             {
                $this->trae_practicas_ge_es[]=$reg;
             }
                return $this->trae_practicas_ge_es;
        }
